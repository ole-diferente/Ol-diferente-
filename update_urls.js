...
const dir = '.';
const files = fs.readdirSync(dir).filter(f => f.endsWith('.html') || f === 'notes_db.js');

files.forEach(file => {
    let content = fs.readFileSync(file, 'utf8');
    content = content.replace(/https:\/\/www\.fragrantica\.com\/ndata\/images\/notes\/(m\.\d+\.jpg)/g, (match, p1) => {
        return urlToLocal[p1] || ('Imagenes_Notas_Olfativas/' + p1);
    });
